...
class ModelSettingsUI {
  constructor(container, data, modelType) {
    console.log('Шаг 7Создание экземпляра ModelSettingsUI - constructor', {
      container,
      modelType,
      dataItems,
    });
  ...
  selectModel(fileName) {
    ...
    console.log('this in selectModel:', this);
    ...
    // Сохраняем состояние текущей модели если она есть
    if (this.activeModel) {
      const currentState = {
        pommel: {
          label: document.querySelector('.group-section[data-title="Pommel"] .color-button.active')?.getAttribute('data-label'),
      ...
      modelColorStates[this.type][currentFileName] = currentState;
    }
    ...
    // Восстанавливаем состояние из сохраненного если оно есть
    requestAnimationFrame(() => {
      const savedState = modelColorStates[this.type][fileName];
