 ...
 class ModelSettingsUI {
   ...
   renderModelSelector() {
     ...
     function extractColorsByGroupId(items) {
       const colorsByGroupId = {};
     
       items.forEach(item => {
         if (item.refGroups && Array.isArray(item.refGroups)) {
           item.refGroups.forEach(group => {
             if (group.groupedNodes && Array.isArray(group.groupedNodes)) {
               group.groupedNodes.forEach(node => {
                 if (node.settingsData && node.settingsData.difmaps) {
                   const colors = [];
                   node.settingsData.difmaps.forEach(difmap => {
                     if (difmap.colors && Array.isArray(difmap.colors)) {
                       colors.push(...difmap.colors);
                     }
                   });
                   
                   if (colors.length > 0) {
                     if (!colorsByGroupId[group.id]) {
                       colorsByGroupId[group.id] = [];
                     }
                     colorsByGroupId[group.id].push(...new Set(colors));
                   }
                 }
               });
             }
       ...
       // Log the results
       console.log('Colors by Group ID:');
       Object.entries(colorsByGroupId).forEach(([groupId, colors]) => {
         console.log(`Group ID ${groupId}:`, colors);
       });
     
       return colorsByGroupId;
     }

     this.data.items && extractColorsByGroupId(this.data.items);
