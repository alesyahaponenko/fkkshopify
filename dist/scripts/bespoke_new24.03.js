      ...
      <div class="bespoke-steps">
        ...
        <script>
              ...
              if (group.name === "Blade") {
                ...
                if (materialSettings) {
                  group.nodes.forEach((nodeName) => {
                    nodeMaterials[nodeName] = materialSettings;
                  });
                }
              } 
              else if (group.name.includes("Logo")) 
                const logoNodes = group.groupedNodes;
                const engravingSelected = document.querySelector('#laserEngraving input:checked')?.id === "laser-engraving";
          
                const defaultMaterial = logoNodes[0]?.settingsData?.difmaps[0];
                let materialSettings = {
                  color: defaultMaterial?.colors[0]?.color || "#b0b0b0", 
                  metalness: defaultMaterial?.metalness || 0.56,
                  roughness: defaultMaterial?.roughness || 0.42,
                  texture: defaultMaterial?.textures?.[0]?.url || null,
                };
                ...
                if (localStorage.getItem('savedColors') === 'yes' && !["Pommel", "Bolster"].includes(group.name)) {
                  group.nodes.forEach((nodeName) => {
                    if (savedColors[nodeName]) {
                      nodeMaterials[nodeName] = savedColors[nodeName];
                    }
                  });
                  return;
                }
                ...
                group.nodes.forEach((nodeName, index) => {
                  ...
                  if (["Pommel", "Bolster", "Discs"].includes(group.groupedNodes[0]?.name)) {
                    ...
                    const selectedIndexes = materials
                      .map((mat, idx) => mat.colors.some(colorObj => colorObj.color === nodeColor) ? idx : -1)
                      .filter(idx => idx !== -1);
          ...
          function selectModels(handleModel, bladeModel) {
            ...
            if (bladeModel) {
              console.log('bladeModel',bladeModel)
              bladeWithMaterials = {
                ...bladeModel,
